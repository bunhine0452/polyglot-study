...
final class FSRS: FSRSAlgorithm, @unchecked Sendable {
    ...
    func `repeat`(
        ...
        let obj = try scheduler(for: card, reviewTime: now)
    ...
    /// Upstream had a third arm — `v5 + enableShortTerm` → the legacy
    /// `BasicScheduler` (hardcoded 1m/5m/10m). `Scheduler/BasicScheduler.swift`
    /// is not vendored (LearnKit is FSRS-6 only), so that arm now throws
    /// `FSRSError(.invalidParam)` instead of silently scheduling under a
    /// different algorithm. `FSRSReviewScheduler` rejects non-21-length `w`
    /// before it can ever get here. See Vendor/FSRS/VENDORING.md.
    /// ────────────────────────────────────────────────────────────────────────
    private func scheduler(for card: Card, reviewTime: Date) throws -> AbstractScheduler {
        ...
        case .v5:
            throw FSRSError(
                .invalidParam,
                "short-term v5 scheduling is not vendored — LearnKit is FSRS-6 only "
                    + "(w.count == \(parameters.w.count), expected 21)"
            )
        case .v6:
    ...
    // ─── learnkit local modification ─────────────────────────────────────────
    // `reschedule(currentCard:reviews:options:)` was removed together with
    // `Scheduler/FSRSReschedule.swift`. LearnKit rebuilds card state from
    // `review_log` through `ReviewLogReplay`, which owns ordering and the
    // manual-rating policy itself; the upstream helper would be a second,
    // divergent replay path. See Vendor/FSRS/VENDORING.md.
    // ─────────────────────────────────────────────────────────────────────────
}
