...
enum CardState: Int, Codable, Sendable {
      case new = 0
      case learning = 1
      case review = 2
      case relearning = 3
...
enum Rating: Int, Codable, Equatable, CaseIterable, Sendable {
...
struct ReviewLog: Equatable, Codable, Hashable, Sendable {
...
struct Card: Equatable, Codable, Hashable, Sendable {
    var due: Date             // Date when the card is next due for review
    var stability: Double     // A measure of how well the information is retained
    var difficulty: Double    // Reflects the inherent difficulty of the card content
    var elapsedDays: Double   // Days since the card was last reviewed
    var scheduledDays: Double // The interval at which the card is next scheduled
    /// 0-based index into `params.learningSteps` / `relearningSteps`. Only
    ...
    var learningSteps: Int
    var reps: Int             // Total number of times the card has been reviewed
    var lapses: Int           // Times the card was forgotten or remembered incorrectly
    var state: CardState      // The current state of the card (New, Learning, Review, Relearning)
    var lastReview: Date?     // The most recent review date, if applicable
...
struct RecordLogItem: Codable, Equatable, Hashable, Sendable {
    var card: Card
    var log: ReviewLog
    
    init(card: Card, log: ReviewLog) {
        self.card = card
        self.log = log
    }
}

typealias RecordLog = [Rating: RecordLogItem]

struct FSRSParameters: Codable, Equatable, Sendable {
...
struct FSRSReview: Codable, Sendable {
    /**
...
struct FSRSState: Codable, Sendable {
