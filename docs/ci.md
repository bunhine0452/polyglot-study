...
## 러너 이미지 — macos-26 이어야 한다 (실측 2026-09-07)

세 패키지가 모두 `swift-tools-version: 6.2` 다. 러너의 기본 툴체인이 그보다 낮으면
SwiftPM 이 곧바로 죽는다. 첫 실 PR 에서 `macos-14` 로 두 게이트가 11초 만에 터졌고,
이미지 매트릭스로 한 번에 확인했다.

| 라벨 | ImageOS | 기본 Swift | 쓸 수 있나 |
|---|---|---|---|
| `macos-14` | macos14 | 5.10 | ✗ (Xcode 최대 16.2 = Swift 6.0.3) |
| `macos-15` | macos15 | 6.1.2 | ✗ |
| `macos-26` | macos26 | **6.3.3** | ✓ 로컬 개발 툴체인과 동일 |
| `macos-latest` | macos26 | 6.3.3 | ✓ 다만 라벨이 드리프트한다 |

`macos-latest` 가 아니라 **`macos-26` 을 명시**한다 — latest 는 언제든 다음 이미지로
옮겨 가고, 그때 조용히 깨지는 것보다 명시적으로 낡는 편이 낫다.
...
## 러너는 로컬보다 훨씬 느리다 — 잡을 갈랐다 (실측 2026-09-07)

`swift test`(LearnKit + Tools)를 한 잡에 몰아넣었더니 **30분 타임아웃에 걸려 취소**됐다.
30분 44초 시점에 `swiftpm-testing` 이 아직 돌고 있었다. 로컬은 21초다 — 차이는 콜드
빌드이고, 특히 Swift 채점 테스트가 SwiftPM 패키지를 굽는 비용이 크다.

같은 실행에서 `build-app.sh` 는 3분 50초, `packtool validate` 는 3분 16초였다.
즉 느린 것은 앱 빌드가 아니라 **테스트 스위트**다.

타임아웃만 60분으로 올리지 않고 **LearnKit 과 Tools 를 별도 잡으로 갈랐다.**

- 병렬로 돌아 벽시계가 준다.
- 신호가 좋아진다 — 한 잡이면 어느 쪽이 느린지·어느 쪽이 깨졌는지 로그를 뒤져야
  알지만, 갈라 두면 체크 이름이 바로 말해 준다.

**주의: 잡이 취소되는 이유는 둘이다.** 타임아웃 초과와 `concurrency.cancel-in-progress`
(PR 에 새 커밋이 오면 낡은 실행을 접는다)가 로그에서 똑같이 `cancelled` 로 보인다.
게이트를 지켜보면서 동시에 push 하면 영영 완주하지 못한다 — 실제로 그렇게 몇 번을
날렸다. 실행 시간(`gh run list` 의 소요)이 타임아웃과 같으면 전자, 훨씬 짧으면 후자다.

## RunnerKit 은 직렬로 돌린다 (실측 2026-09-07)

프로세스를 실제로 띄우는 스위트라 병렬로 돌리면 러너에서 **물린다.**

| 실행 방식 | 결과 |
|---|---|
| 병렬(기본) | 25분 상한 초과. 로그가 19분간 침묵, 34개 스위트 중 0개 종료 |
| `--no-parallel` | **8분 47초 통과** |

같은 코드·같은 러너다. 특정 테스트가 매달리는 게 아니다 — 미완료 목록에 프로세스를
안 쓰는 순수 SQL 비교 테스트까지 들어 있었는데, 그건 동시성 풀이 고갈돼 스케줄조차
안 된 것이다. 동시에 수십 개가 로그인 셸(`zsh -lic`)·`sandbox-exec`·`swiftc` 를
띄우면서 물린다. `RLIMIT_NPROC` 이 프로세스 트리가 아니라 **uid 전체**를 센다는
실측(`HANDOFF.md`)과 맞물리는 자리다.

`LanguageToolchain` 주석에 같은 종류의 흔적이 이미 있다 — "8개를 동시에 돌리면
`zsh -lic` 로그인 셸이 8번 뜨고, 실측에서 그 폭풍이 실행 하나를 30초 넘게 밀어냈다."
앱은 진행 중인 Task 를 캐시해 막았지만 **테스트는 각자 독립이라 그 캐시를 공유하지 않는다.**

직렬화는 회피가 아니라 이 스위트에 맞는 실행 방식이다. 다만 "동시에 프로세스를 많이
띄우면 물린다" 자체는 테스트만의 문제가 아닐 수 있어 별도 항목으로 남긴다.

### 잡을 넷으로 가른 이유

한 잡이면 "뭔가 30분 넘게 걸린다" 까지만 알 수 있다. 갈라 두면 체크 이름이 바로
범인을 말한다 — 실제로 이번 진단이 그 방식으로 두 번의 왕복 만에 끝났다.

실측 소요(2026-09-07): RunnerKit 8분 47초 · LearnKit 나머지 6분 2초 ·
Tools 5분 19초 · build-app 4분 5초 · packtool validate 4분 39초. 병렬이라 벽시계는
가장 느린 잡 하나에 수렴한다.
